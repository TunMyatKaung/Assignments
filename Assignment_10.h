...
int phone_found=-1;
int trans_limit=0;
unsigned int total_currentday_amount=0;
...
unsigned int calculate_trans_amounts_same_days(int to_calcu_index);
unsigned int calculate_receipt_amounts_same_days(int to_calcu_index);
...
void userSector(){
    ...
    if(user_option==1){
        ...
        current_data_toTransfer(amount_to_transfer);
        get_limit_amount(emailExist);
        calculate_trans_amounts_same_days(emailExist);
        get_limit_amount(phone_found);
        calculate_receipt_amounts_same_days(phone_found); //transaction limit amount per day for receiver
...
unsigned int calculate_trans_amounts_same_days(int to_calcu_index){
    //current_day_to_transfer=21;
    int record_counter =space_array[to_calcu_index]-15;
    int index_counter=0;
    total_currentday_amount=0;
    ...
    for(int a=record_counter-1; a>0 ; a--) {
        ...
        if (db[to_calcu_index].trc[a].note[0] == 'F') {
            // to get $
            ...
            if (current_record_day == current_day_to_transfer) {
                ...
                printf("current_transferred_amount: %d\n", current_record_amount); // trc history of current day
                total_currentday_amount += current_record_amount; //to get total amount of current day
            } else {
                break;
            }
        ...
        printf("total_current_transfer_amount: %d\n\n",total_currentday_amount); //total amount of current day

        if(trans_limit<total_currentday_amount+current_amount_to_transfer){
                    printf("..transaction limit over!!..\n\n");
                    userSector();
        }
}
...
unsigned int calculate_receipt_amounts_same_days(int to_calcu_index){
    ...
    for(int a=record_counter-1; a>0 ; a--){
        ...
        if (db[to_calcu_index].trc[a].note[0] != 'F') {
            ...
            if (current_record_day == current_day_to_transfer) {
                unsigned int current_record_amount = charArray_to_unsigned_fun(amount_char_array);
                printf("current_received_amount of %s: %d\n",db[to_calcu_index].name, current_record_amount); // trc history of current day
                total_currentday_amount += current_record_amount; //to get total amount of current day
            } else {
                break;
            }
    ...
    printf("total_current_received_amount of %s : %d\n\n",db[to_calcu_index].name,total_currentday_amount); //total amount of current day

    if(trans_limit<total_currentday_amount+current_amount_to_transfer){
        printf("..cannot receive your transfer!! the acc %s limit is over!!..\n\n",db[to_calcu_index].name);
        userSector();
    }
