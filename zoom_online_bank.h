...
#define ASSIGNMENTPROJ_ZOOM_ONLINE_BANK_H
...
// from-WinHtut-to-lonelone-100
    ...
    time_t t;
    char dt[90]; //dt == date & time
    struct tm *tptr;

    time(&t);
    tptr=localtime(&t);
    strftime(dt,90,"%x-%I:%M%p",tptr);
    int dt_count= charCounting(dt);
    ...
    if(who == 't'){
        ...
        for(int x=0; x<dt_count; x++){
            db[transfer].trc[space_array[transfer]-15].note[index_point]=dt[x];
            index_point++;
        }
        ...
        for(int x=0; x<dt_count; x++){
            db[receiver].trc[space_array[receiver]-15].note[index_point]=dt[x];
            index_point++;
        }
...
#endif //ASSIGNMENTPROJ_ZOOM_ONLINE_BANK_H
